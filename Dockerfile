...
FROM builder AS cmake
	...
	RUN set -ex \
	    ...
	    && mkdir /out \
	    && ./configure --parallel=$(cat /nproc) --prefix=/out \
	    && make -j$(cat /nproc) \
	    && make install
...
# Monero
FROM builder AS monerod


	COPY --from=cmake   	/out 	/usr/local
...
FROM ubuntu AS runtime
	...
	COPY --from=monerod /src/build/release/bin /usr/local/bin/
